...
export const slides: Slide[] = [
  // --- PART 1: FITTING THE LINE ---
  {
    ...
    paragraphs: [
      ...
      "There are two libraries that fit regression models in Python, and it is natural to assume they are competing methods that might give different answers. They are not. Both fit exactly the same line — the one that minimises the residual sum of squares — using exactly the same math you did by hand. What differs is what each library was designed for, and therefore what it asks of you and what it reports back.",
      "Scikit-learn is built for prediction. Its interface is the same for every model it offers: you call `.fit(X, y)` to learn the coefficients, then `.predict(X)` to get predictions for new rows. It reports the slope and intercept, and that is about all. It does not tell you how uncertain the slope is, or whether the relationship could be a coincidence. That is not an oversight; those questions are simply outside its purpose.",
  ...
  // --- PART 2: DID WE JUST GET LUCKY? ---
  {
    ...
    paragraphs: [
      ...
      "So the empirical p-value is zero out of 5,000 — none of the shuffles came anywhere close. In practice we would report p < 0.0002, since the smallest p-value a simulation can measure is one divided by the number of shuffles. The conclusion is that the null hypothesis is not a credible explanation: the relationship between flipper length and mass is not the kind of thing chance produces.",
    ],
    keyTakeaways: [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Now comes the part where nearly everyone slips, including working scientists. It is very tempting to say 'there is a 95% probability that the true slope lies between 47.1 and 53.2.' That sentence sounds right and is wrong. The true slope of the population is a single fixed number. It does not have a probability of being anywhere; it is either inside our interval or it is not, and we do not know which.",
      "What the 95% actually describes is the procedure, not this particular interval. If we repeated the whole study many times — new sample, new fit, new interval each time — about 95% of the intervals we produced would contain the true slope, and about 5% would miss it. Our one interval is one of those. It is probably one of the 95% that captured the truth, but there is no way to tell from the inside.",
      "Press 'What does 95% mean?' to watch this happen. The widget draws forty fresh samples from a population with a known slope and builds forty intervals, all by the same recipe. Around 38 of them cover the true value and around 2 miss. The ones that miss look exactly like the ones that hit — same width, same appearance. That is the honest content of a confidence interval: a good procedure, applied once.",
    ],
    keyTakeaways: [
      ...
      'An interval that excludes zero is significant at α = 0.05 — the same verdict as the p-value, with more information attached.',
    ],
    formula: {
  ...
  },
  {
    ...
    paragraphs: [
      "We now have three tools — the p-value, the standard error and the confidence interval — and they are all connected through the sample size. This slide is about a consequence of that connection which is easy to state and surprisingly easy to forget: whether a result is 'statistically significant' depends as much on how much data you collected as on how large the effect really is.",
